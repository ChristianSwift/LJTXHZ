...
if( ! function_exists( 'wtr_header' ) ){
	...
	function wtr_header(){
		...
		$full_width_navigation	= ( 1 == $post_settings['wtr_HeaderFullwidthNavigation'] ) ? 'wtrFullWidthHeader': '';
		...
		switch ( $LogoPosition ) {
			...
			case '1': // Right
				...
				$menu_class				= "wtrMainNavigation clearfix";
				$logo_position_class	= "wtrLogoReverse";
				break;
		}
		...
		echo '<header class="wtrHeader wtrAnimate wtrHeaderColor' . $header_type_class . ' ' . $transparent_class . ' ' . $simplified_class . ' ' . $clean_menu_class . ' ' . $full_width_navigation . ' ' . $logo_position_class . ' " '. $transparent_data . ' '. $navigation_type_data .'>';
			wtr_header_section();
		...
		if( '1' == $navigation_type && '0' == $transparent_mode ){
			...
			echo '<header class="wtrHeaderSecond wtrDisplayHide wtrHeaderFixed wtrAnimate wtrHeaderColor ' . $header_type_class . ' ' . $top_class . '  ' . $simplified_class . ' ' . $clean_menu_class . ' ' . $menu_primary_class . ' ' . $full_width_navigation . ' ' . $logo_position_class . ' " '. $transparent_data . ' '. $navigation_type_data .'>';
				wtr_header_section();
				echo '<div class="wtrMainHeader">';
...
if ( ! function_exists( '_wp_render_title_tag' ) ) {
	...
	// backwards compatibility for older versions to generating a title tag
	function wtr_render_title_tag() {
		echo '<title>';
			wp_title( '', true );
		echo '</title>';
	} // end wtr_render_title_tag
	add_action( 'wp_head', 'wtr_render_title_tag' );
}
...
if( ! function_exists( 'wtr_body_class' ) ){
	...
	function wtr_body_class( $classes ) {
		...
		if ( ( is_single() OR  is_page() ) AND ! empty( $post_settings['wtr_CustomeBodyExtraClass'] ) ) {
			$classes[] = $post_settings['wtr_CustomeBodyExtraClass'];
		}
		return $classes;
	}
